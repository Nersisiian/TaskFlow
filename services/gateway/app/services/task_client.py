...
class TaskServiceClient:
    ...
    async def get_tasks(self, user_id: str, params: dict = None):
        async with aiohttp.ClientSession() as session:
            headers = {"X-User-ID": user_id}
            async with session.get(
                f"{self.base_url}/api/v1/tasks", headers=headers, params=params
            ) as resp:
                if resp.status != 200:
    ...
    async def create_task(self, payload: dict, user_id: str):
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                f"{self.base_url}/api/v1/tasks", json=payload, headers=headers
            ) as resp:
                if resp.status != 201:
